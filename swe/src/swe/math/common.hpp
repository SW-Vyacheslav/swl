#ifndef SWE_MATH_COMMON_HPP_
#define SWE_MATH_COMMON_HPP_

#include <cstdint>

namespace swe
{

template<std::size_t Size, typename T> struct vec;

template<typename T> using vec1 = vec<1, T>;
template<typename T> using vec2 = vec<2, T>;
template<typename T> using vec3 = vec<3, T>;
template<typename T> using vec4 = vec<4, T>;

#define SWE_MATH_DEFINE_VEC_TYPES(t, l) \
    using vec1 ## l = vec<1, t>;        \
    using vec2 ## l = vec<2, t>;        \
    using vec3 ## l = vec<3, t>;        \
    using vec4 ## l = vec<4, t>;

SWE_MATH_DEFINE_VEC_TYPES(double, d)
SWE_MATH_DEFINE_VEC_TYPES(float, f)
SWE_MATH_DEFINE_VEC_TYPES(std::uint64_t, u64)
SWE_MATH_DEFINE_VEC_TYPES(std::int64_t, i64)
SWE_MATH_DEFINE_VEC_TYPES(std::uint32_t, u32)
SWE_MATH_DEFINE_VEC_TYPES(std::int32_t, i32)
SWE_MATH_DEFINE_VEC_TYPES(std::uint16_t, u16)
SWE_MATH_DEFINE_VEC_TYPES(std::int16_t, i16)
SWE_MATH_DEFINE_VEC_TYPES(std::uint8_t, u8)
SWE_MATH_DEFINE_VEC_TYPES(std::int8_t, i8)

#undef SWE_MATH_DEFINE_VEC_TYPES

template<std::size_t Rows, std::size_t Cols, typename T> struct mat;

template<typename T> using mat2x2 = mat<2, 2, T>;
template<typename T> using mat2x3 = mat<2, 3, T>;
template<typename T> using mat2x4 = mat<2, 4, T>;
template<typename T> using mat3x2 = mat<3, 2, T>;
template<typename T> using mat3x3 = mat<3, 3, T>;
template<typename T> using mat3x4 = mat<3, 4, T>;
template<typename T> using mat4x2 = mat<4, 2, T>;
template<typename T> using mat4x3 = mat<4, 3, T>;
template<typename T> using mat4x4 = mat<4, 4, T>;

#define SWE_MATH_DEFINE_MAT_TYPES(t, l) \
    using mat2x2 ## l = mat<2, 2, t>;   \
    using mat2x3 ## l = mat<2, 3, t>;   \
    using mat2x4 ## l = mat<2, 4, t>;   \
    using mat3x2 ## l = mat<3, 2, t>;   \
    using mat3x3 ## l = mat<3, 3, t>;   \
    using mat3x4 ## l = mat<3, 4, t>;   \
    using mat4x2 ## l = mat<4, 2, t>;   \
    using mat4x3 ## l = mat<4, 3, t>;   \
    using mat4x4 ## l = mat<4, 4, t>;

SWE_MATH_DEFINE_MAT_TYPES(double, d)
SWE_MATH_DEFINE_MAT_TYPES(float, f)
SWE_MATH_DEFINE_MAT_TYPES(std::uint64_t, u64)
SWE_MATH_DEFINE_MAT_TYPES(std::int64_t, i64)
SWE_MATH_DEFINE_MAT_TYPES(std::uint32_t, u32)
SWE_MATH_DEFINE_MAT_TYPES(std::int32_t, i32)
SWE_MATH_DEFINE_MAT_TYPES(std::uint16_t, u16)
SWE_MATH_DEFINE_MAT_TYPES(std::int16_t, i16)
SWE_MATH_DEFINE_MAT_TYPES(std::uint8_t, u8)
SWE_MATH_DEFINE_MAT_TYPES(std::int8_t, i8)

#undef SWE_MATH_DEFINE_MAT_TYPES

} // namespace swe

#endif // SWE_MATH_COMMON_HPP_
